 ...
 *
 * Description:
 * This function examines  'buf' buffer starting from position 'i' and checks
 * for the presence of  command chain delimiter (';'). If found, it updates 'j'
 * to point to the position immediately after the delimiter and adjusts 'i' and
 * 'len' as necessary.
 *
 * Return:
 * This function wont return a value explicitly, but it updates 'j', 'i', and
 ...
 * the '#' character. It modifies 'buf' in-place by replacing the comment
 * and the newline character that follows it with a null terminator, effectively
 ...
 * @sig_num: The signal number (should be SIGINT)
 *
 * Description:
 * This function is a signal handler for the SIGINT signal, which is typically
 * generated when the user presses Ctrl-C. It provides a simple action to be
 * taken when the signal is received, which is to print a newline and a shell
 * prompt.
 *
 * Return:
 ...
 *
 * Description:
 * This function reads input from the standard input (stdin) and buffers it in
 * the provided buffer. It handles various aspects of input processing, such as
 * removing trailing newlines and comments, and tracking line counts.
 *
 * Return:
 ...
 * a previously buffered input. It handles various aspects of input processing,
 * including command chaining and line processing.
 *
 * Return:
 * Returns the number of bytes read or -1 on error.
 */
 ...
 *
 * Description:
 * The `main` function is the entry point of the shell program. It manages the
 * main loop of the shell, which repeatedly prompts the user input, processes
 * the input, and executes commands. The shell continues to run until the user
